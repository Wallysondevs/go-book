  ...
  return (
    <PageContainer title={ch.title} subtitle={ch.subtitle} difficulty={ch.difficulty}>
      ...
      <nav className="mt-12 pt-6 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between gap-4">
        {prev ? (
          <Link href={`/c/${prev.slug}`} className="flex-1 group p-3 rounded-lg border border-slate-200 dark:border-slate-800 hover:border-go-cyan transition-colors">
            <div className="flex items-center gap-1 text-xs text-slate-500"><ArrowLeft size={12} /> Anterior</div>
            <div className="text-sm font-semibold text-go-cyan-dark dark:text-go-cyan truncate">{prev.title}</div>
          </Link>
        ) : <div className="flex-1" />}
        {next ? (
          <Link href={`/c/${next.slug}`} className="flex-1 group p-3 rounded-lg border border-slate-200 dark:border-slate-800 hover:border-go-cyan transition-colors text-right">
            <div className="flex items-center justify-end gap-1 text-xs text-slate-500">Próximo <ArrowRight size={12} /></div>
            <div className="text-sm font-semibold text-go-cyan-dark dark:text-go-cyan truncate">{next.title}</div>
          </Link>
        ) : <div className="flex-1" />}
      </nav>
    </PageContainer>
